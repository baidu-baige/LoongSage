...
- **高可扩展的插件系统** — 提供多达 12 个核心扩展点（涵盖智能体、奖励模型、沙箱、优势估计、策略损失、KL 散度、路由中间件、数据过滤及异步调度策略等），只需将新增实现放入 `coda/custom/` 目录并在配置中引用，即可热插拔生效，无需改动框架主干代码。
...
| [自定义扩展](docs/zh/custom-extensions.md) | 所有扩展点的统一存放目录与加载机制 |
