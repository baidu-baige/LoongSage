...
- `agent/claude_code/claude_code_agent.py` — black-box Claude Code agent, driving the Claude Code CLI inside a sandbox to complete SWE tasks (registered name `claude_code`). It speaks the **Anthropic Messages protocol** (`/v1/messages`) and is configured **entirely through environment variables**, with no `config.toml` to write like codex: `ANTHROPIC_BASE_URL` points at the Router, `ANTHROPIC_MODEL` (and the Haiku/Sonnet/Opus per-tier defaults) are all set to `coda-actor`, and nonessential traffic, background tasks, the auto-updater, the attribution header, and retries are disabled (`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` / `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` / `DISABLE_AUTOUPDATER` / `CLAUDE_CODE_ATTRIBUTION_HEADER=0` / `CLAUDE_CODE_MAX_RETRIES=0`), with `IS_SANDBOX=1` so that `--permission-mode bypassPermissions` is allowed under root. When `max_output_tokens>0` (default 32000) it sets `CLAUDE_CODE_MAX_OUTPUT_TOKENS` to align the CLI's per-request `max_tokens` with the rollout budget; when `context_length>0` it sets `CLAUDE_CODE_AUTO_COMPACT_WINDOW` from `auto_compact_ratio` (default 0.8) to trigger Claude's own auto-compaction. `run_trajectory()` first writes two files via base64-encoded `printf`: a **PreToolUse hook** shell script (`/tmp/coda_read_limit_hook.sh`) and a settings file that references it (`/tmp/coda_claude_settings.json`); the hook rewrites an unbounded/oversized `Read` tool call into a bounded read with a `limit` (raw byte budget 12500, fallback 250 lines, keeping a single `tool_result` around 15k characters) so a big-file read cannot blow up the context window and trigger the CLI's auto-compaction (a frequent cause of exit-code-1 crashes). The instruction is then fed via stdin (`printf | claude`) and the task runs in one shot via `claude --print --verbose --output-format stream-json --permission-mode bypassPermissions --settings ... --disallowedTools WebFetch,WebSearch`, redirecting logs to `/tmp/coda-claude-code.log`. Before launching it likewise runs a `/dev/tcp` connectivity check via `_probe_router()`. On a non-zero exit code it reads back a `ps` process snapshot and the last 100 log lines: if the exit code is 1 and the log matches context-length-exceeded signatures (`CONTEXT_LENGTH_EXCEEDED` or context exceed/exhaust), it **degrades gracefully** by returning `Reward(final_reward=0.0, is_valid=True, is_correct=False)` instead of raising (avoiding dropping the whole trajectory group); otherwise it raises a `RuntimeError`. `close()` interrupts a still-running `claude` process with `pkill -INT` (then `pkill -KILL` after a 1s grace period).
