...
- `agent/claude_code/claude_code_agent.py` —— 黑盒 Claude Code agent，在沙箱内驱动 Claude Code CLI 完成 SWE 任务（注册名 `claude_code`）。它走 **Anthropic Messages 协议**（`/v1/messages`），且**完全通过环境变量**配置，无需像 codex 那样写 `config.toml`：`ANTHROPIC_BASE_URL` 指向 Router，`ANTHROPIC_MODEL`（以及 Haiku/Sonnet/Opus 各档默认模型）统一设为 `coda-actor`，并关闭非必要流量、后台任务、自动更新、归因头与重试（`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` / `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` / `DISABLE_AUTOUPDATER` / `CLAUDE_CODE_ATTRIBUTION_HEADER=0` / `CLAUDE_CODE_MAX_RETRIES=0`），`IS_SANDBOX=1` 以允许 root 下的 `--permission-mode bypassPermissions`。若 `max_output_tokens>0`（默认 32000）设置 `CLAUDE_CODE_MAX_OUTPUT_TOKENS` 让 CLI 的单请求 `max_tokens` 与 rollout 预算对齐；若 `context_length>0` 按 `auto_compact_ratio`（默认 0.8）设置 `CLAUDE_CODE_AUTO_COMPACT_WINDOW` 触发 Claude 自身的自动压缩。`run_trajectory()` 先以 base64 编码 `printf` 写入两个文件：一个 **PreToolUse hook** shell 脚本（`/tmp/coda_read_limit_hook.sh`）和引用它的 settings（`/tmp/coda_claude_settings.json`）；该 hook 会把无界/超大的 `Read` 工具调用重写成带 `limit` 的有界读取（原始字节预算 12500、回退 250 行，令单个 `tool_result` 约 15k 字符），避免大文件读取撑爆上下文、触发 CLI 自动压缩（这是 exit-code-1 崩溃的常见诱因）。随后指令经 stdin（`printf | claude`）喂入，以 `claude --print --verbose --output-format stream-json --permission-mode bypassPermissions --settings ... --disallowedTools WebFetch,WebSearch` 子进程一次性跑完，日志重定向到 `/tmp/coda-claude-code.log`。启动前同样用 `_probe_router()` 做 `/dev/tcp` 连通性探测。非 0 退出码时先回读 `ps` 进程快照与末尾 100 行日志：若退出码为 1 且日志命中上下文超限特征（`CONTEXT_LENGTH_EXCEEDED` 或 context exceed/exhaust），则**降级返回** `Reward(final_reward=0.0, is_valid=True, is_correct=False)` 而非抛错（避免整组 trajectory 被丢弃）；否则抛出 `RuntimeError`。`close()` 通过 `pkill -INT`（宽限 1s 后 `pkill -KILL`）中断仍在运行的 `claude` 进程。
