...
This module also holds the public channel API: ``ChannelMeta`` (the configuration
callers fill in) together with ``create_sender_channel`` /
``create_receiver_channel``, which manage one process-local channel instance.
"""
...
def _classic_ipc_segments():
    ...
    When ``PYTORCH_ALLOC_CONF=expandable_segments:True`` is in effect,
...
# ---------------------------------------------------------------------------
# Process-local channel lifecycle
#
...
#           [+ train_world_size for receivers]
# ---------------------------------------------------------------------------
...
def _get_ip_from_ray() -> str | None:
    ...
    Ray is imported lazily so that this module stays importable in processes
    that never join a Ray cluster.
    """
    try:
        import ray
